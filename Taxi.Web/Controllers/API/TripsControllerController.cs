...
namespace Taxi.Web.Controllers.API
{
    ...
    public class TripsController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddIncident([FromBody] IncidentRequest request)
        {
            ...
            if (taxiEntity == null)
            {
                _context.Taxis.Add(new TaxiEntity { Plate = request.Plate.ToUpper() });
        ...
        public async Task<IActionResult> PostTripEntity([FromBody] TripRequest tripRequest)
        {
            ...
            TaxiEntity taxiEntity = await _context.Taxis.FirstOrDefaultAsync(t => t.Plate == tripRequest.Plate);
            if (taxiEntity == null)
            {
                _context.Taxis.Add(new TaxiEntity { Plate = tripRequest.Plate.ToUpper() });
                await _context.SaveChangesAsync();
                taxiEntity = await _context.Taxis.FirstOrDefaultAsync(t => t.Plate == tripRequest.Plate);
            }
